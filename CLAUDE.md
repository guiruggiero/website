...
Neither page has an in-page `<h1>` — page-level controls go in the header's right slot, where public pages carry social links (`Refresh` plus the last-checked time on `admin.html`; Firebase Console and Sign out on `admin-firestore.html`), and the tabs between the two pages are the first thing under it. `admin.css` overrides `main` to the site's standard 750px so both pages line up with the header and footer.
...
`modules/admin-status.js` polls each service's health endpoint on page load and on the header's `Refresh` button — never on an interval — and paints a status pill (dot + label) plus a detail line per card. When a service is up, that line carries only the commit hash, since the pill already says so. GuiDo and claudeCodeGateway are live checks; details on those endpoints live in their own repos, not here. Both cards link out to the same PM2 bucket via an icon in the card header.
