...
Neither page has an in-page `<h1>` — page-level controls go in the header's right slot, where public pages carry social links (`Refresh` plus the last-checked time on `admin.html`; a Firebase Console link on `admin-firestore.html`), and the tabs between the two pages are the first thing under it. `admin.css` overrides `main` to the site's standard 750px so both pages line up with the header and footer.
...
`modules/admin-firestore.js` uses the **full** Firestore SDK rather than the Lite build in `modules/firebase.js`, because it needs `getDocs` queries. It reuses the app instance exported by `admin-auth.js` instead of initializing a second one. User turns are rendered with `textContent`, never `innerHTML` — unsanitized input from anonymous visitors. Model turns are rendered with `innerHTML` through DOMPurify, using the same tag/attribute allowlist as `ui.js`'s chat window — GuiPT's replies carry HTML (links, line breaks) that needs to render, not display as raw markup. The viewer is **read-only by design** — no delete button — each chat row shows its Firestore document ID so a chat can be found and deleted from the Firebase Console instead (`Ctrl+F` the ID there; the badge is click-to-copy). Filters are the collection, a date range with 7d/30d/90d presets that query on click, and an expand/collapse-all toggle over the result list.

### Service Health Dashboard

`modules/admin-status.js` polls each service's health endpoint on page load and on the header's `Refresh` button — never on an interval — and paints a status pill (dot + label) plus a detail line per card. When a service is up, that line shows the commit hash as a copyable code badge, same pattern as the chat-ID badge below. GuiDo and claudeCodeGateway are live checks; details on those endpoints live in their own repos, not here. Both cards link out to the same PM2 bucket via an icon in the card header.

On Air sign is a placeholder/mock card, and can't be a live one from here: the Pico serves plain HTTP on the LAN while this page is HTTPS, so the browser blocks its `/stats`, `/logs` and `/errors` as mixed content — no CORS header on the Pico can change that. Its links still work from home, since a top-level navigation isn't mixed content.
