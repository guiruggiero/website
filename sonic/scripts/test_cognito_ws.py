#!/usr/bin/env python3
"""
Test WebSocket connection using Cognito unauthenticated credentials
(same flow as the browser). This isolates signing vs permissions issues.
"""
import asyncio
import boto3
...
def get_cognito_credentials():
    """Get temporary credentials via Cognito basic (classic) flow + STS AssumeRoleWithWebIdentity."""
...
def get_presigned_url(cognito_creds):
    """Sign URL with Cognito temp creds using botocore (same algorithm as test_ws.py)."""
    creds = Credentials(
